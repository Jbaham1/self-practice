...
function endsWithVowel(str){
    var vowels = ["a","e","i","o","u"]
    return vowels.indexOf(str[str.length -1]) !== -1;
}
...
function median(input){
    var median = 0;
    var numsLen = input.length;
    input.sort();
    if (numsLen % 2 === 0) {
        // average of two middle numbers
        median = (input[numsLen / 2 - 1] + input[numsLen / 2]) / 2;
    } else {
        median = input[(numsLen - 1) / 2];
    }
...
var getNumberOfBooks = function(arr){
    return arr.length;
}
...
function getAverageItemPrice(obj){
    ...
    return (total/ shoppingCart.items.length)/2;
}
...
function getAverageSpentPerItem(obj){
    ...
    for(var key in obj){
        for(var i = 0; i < shoppingCart.items.length; i++){
            ...
            totalItems += shoppingCart.items[i].quantity/2
        }
    }

    return totalPrice / totalItems /2
}
...
function mostSpentOnItem(obj){
    var most = {"price": 0, "quantity":0}
    for(var key in obj){
        var items = obj.items
        for(var i = 0; i < items.length; i++){
            if(items[i].quantity * items[i].price > most.quantity * most.price){
                most = items[i];
            }
        }
    }
